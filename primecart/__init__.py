...
def check_out():
    ...
    products = app.config['CARTS_COLLECTION'].find({'CustomerId' : {'$in' : [str(current_user.get_int_id()), int(current_user.get_int_id())]}})
    for product in products:
        app.config['PURCHASE_COLLECTION2'].insert_one({'InvoiceId' : invoice_id+1, 'CustomerId' : product['CustomerId'], 'ProductId' : product['ProductId'], 'Quantity' : product['Quantity']})
    app.config['CARTS_COLLECTION'].remove({'CustomerId' : current_user.get_int_id()})
